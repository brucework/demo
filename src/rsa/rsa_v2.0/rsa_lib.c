...
char* decodeMessage(int len, int bytes, int* cryptogram, int exponent, int modulus) {
	char *decoded = (char *)malloc(len * sizeof(int));
	int x, i, j;
...
/**
 * genkey function will generate the public(N,e) & private key(N,d).
...
void genkey(int p, int q, _RSA *key)
{
	...
	key->p = p;
	key->phi = (q-1)*(p-1);
	key->e = randExponent(key->phi, EXPONENT_MAX);
	key->d = inverse(key->e, key->phi);
